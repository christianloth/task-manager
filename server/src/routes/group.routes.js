...
router.get("/", (req, res) => {
    MainDB.db.all("SELECT group_id, group_name FROM groups LIMIT 100", (err, rows) => {
        if (err) return err;
        res.json({
            rows,
        });
...
async function getGroup(group_id){
    try{
        ...
        console.log(`Selected Group: "${group["group_name"]}" (ID: ${group_id})!`);
        return group;
    }catch (e) {
...
router.get("/:group_id", async (req, res) => {
    try{    
        ...
        if(!group) res.status(400).send(`Group ${req.params.group_id} is NOT Found`);
        else{
            const members_ids = await getMemberIds(req.params.group_id)
            if((members_ids.includes(req.session.user_id)) || req.body["force"]){
...
router.post("/create", async (req, res) => {
    try{
        ...
        console.log(`Group "${group_name}" with GroupID ${group_id} has been created!`);

        //Create Admin Entry From Owner (user_id)

        res.send(`Successfully Create Group "${group_name}" (GroupID: ${group_id})`);
    }catch (e) {
...
router.put("/:group_id/settings", async (req, res) => {
    try{
        if(await hasAccess(req)){
            const { group_id=req.params.group_id, user_id, group_name, description, icon } = req.body;
            const group = await getGroup(req.params.group_id);
            if(!group){ res.status(400).send(`Group ${req.params.group_id} is NOT Found`); }
            else{
                update_query = "UPDATE groups SET "
                additions = []
                if(group["user_id"] != user_id) additions.push(`user_id = "${user_id}"`);
                if(group["group_name"] != group_name) additions.push(`group_name = "${group_name}"`);
                if(group["descriptions"] != description) additions.push( `descriptions = "${description}"`);
                if(group["icon"] != icon) additions.push(`icon = "${icon}"`);
                for(var i=0; i<additions.length-1; i++){
                    update_query = update_query + additions[i] + ", ";
                }
                update_query = update_query + additions[additions.length-1] + ` where group_id = ${group["group_id"]}`;
                console.log(update_query)

                await MainDB.db.query(update_query)
                res.send(`Updated GroupID(${group_id}) Attributes: \n${additions}`)
            }
        }else{
...
router.delete("/:group_id", async (req, res) => {
    try{
        if(await hasAccess(req)){
            ...
            console.log(`Successfully Deleted Group with GroupID: ${req.params.group_id}`)
            ...
            res.send(`Successfully Deleted Group with GroupID: ${req.params.group_id}`)
        }else{
            res.status(400).send("Insufficient permissions to access this page");
        }
    }catch (e) {
