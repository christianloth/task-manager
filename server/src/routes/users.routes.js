...
async function getUser(user_id) {
    try {
        ...
        console.log(
            `Selected User ${user["username"]} (UserID: ${user["user_id"]})!`
        );
        return user;
    } catch (e) {
    ...
    try {
        ...
        if (!user) {
            ...
            if (hasAccess(req)) {
                ...
                const user_events = getUserRelated(
                    "event_id,event_name",
                    "events",
                    req.params.userId
                );
                const group_ids = getUserRelated(
                    "group_id",
                    "group_member_list",
                    req.params.userId
                );
                const category_ids = getUserRelated(
                    "category_id",
                    "category_member",
                    req.params.userId
                );
                ...
                const user_groups = getNamesFromIds(
                    data[2],
                    "group_id, group_name",
                    "groups",
                    "group_id"
                );
                const user_categories = getNamesFromIds(
                    data[3],
                ...
                answer["1"] =
                    "Insufficient permissions to Access Password or Other User-Related Information";
                res.json(answer);
            }
        }
    } catch (e) {
...
router.post("/create", async (req, res) => {
    try {
        ...
        console.log(
            `User "${username}" with UserID ${user_id} has been created!`
        );
        ...
        if (hasAccess(req) || req.body["force"]) {
            ...
            if (!user) {
                ...
                if (user["username"] != username)
                    additions.push(`username = "${username}"`);
                if (user["first_name"] != first_name)
                    additions.push(`first_name = "${first_name}"`);
                if (user["last_name"] != last_name)
                    additions.push(`last_name = "${last_name}"`);
                if (user["pass_word"] != pass_word)
                    additions.push(`pass_word = "${pass_word}"`);
                if (user["email"] != email)
                    additions.push(`email = "${email}"`);
                ...
                res.send(
                    `Updated UserID(${user_id}) Attributes: \n${additions}`
                );
            }
        } else {
            res.status(400).send(
...
router.delete("/:userId", async (req, res) => {
    try {
        if (hasAccess(req) || req.body["force"]) {
            ...
            console.log(
                `Successfully Deleted User with UserID: ${req.params.userId}`
            );
            req.session.destroy();
